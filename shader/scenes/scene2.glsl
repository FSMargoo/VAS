...
//
float AngelBox(in vec2 Point, in BoxData Box) {
    vec2 center = (Box.RightBottom + Box.LeftTop) / 2.0f;
...
//
float AngelTriangle(in vec2 P, in TriangleData Triangle) {
    vec2 A = Triangle.P1;
    vec2 B = Triangle.P2;
    vec2 C = Triangle.P3;
...
void main()
{
    ...
    BoxData        rectangleBlocks[blocks];
    RectangleLight rectangleLight;
    TriangleLight  triangleLight;

    rectangleLight.Data.LeftTop     = vec2(0.2, 0.2) + vec2(0.2, 0.2);
    rectangleLight.Data.RightBottom = vec2(0.3, 0.4) + vec2(0.2, 0.2);
    rectangleLight.Color            = vec3(0.03529411764705882, 0.24705882352941178, 0.7058823529411765) + vec3(0.6f);

    triangleLight.Data.P1 = vec2(0.15 + 0.1 * (sin(iTime)), 0.3 + 0.2 * (sin(iTime)));
    triangleLight.Data.P2 = vec2(0.05, 0.4);
    triangleLight.Data.P3 = vec2(0.12, 0.2);
    triangleLight.Color   = vec3(0.9294117647058824, 0.20784313725490197, 0.0);

    vec3 color = vec3(0.0);

    rectangleBlocks[0].LeftTop     = vec2(0.3, 0.3);
    rectangleBlocks[0].RightBottom = vec2(0.6, 0.35);

    rectangleBlocks[1].LeftTop     = vec2(0.3, 0.3);
    rectangleBlocks[1].RightBottom = vec2(0.31, 0.65);

    rectangleBlocks[2].LeftTop     = vec2(0.3, 0.65);
    rectangleBlocks[2].RightBottom = vec2(0.6, 0.75);

    rectangleBlocks[3].LeftTop     = vec2(0.6, 0.32);
    rectangleBlocks[3].RightBottom = vec2(0.61, 0.50);

    rectangleBlocks[4].LeftTop     = vec2(0.6, 0.55);
    rectangleBlocks[4].RightBottom = vec2(0.61, 0.65);

    do {
    float rectLightD = SDFBox(uv, rectangleLight.Data);
    float triLightD  = SDFTriangle(uv, triangleLight.Data);
    float boxBlockD[blocks];

    // rectangleBlocks[3].LeftTop     = vec2(0.3, 0.65);
    // rectangleBlocks[3].RightBottom = vec2(0.6, 0.75);
    bool flag = false;
    for (int i = 0; i < blocks; ++i) {
        boxBlockD[i] = SDFBox(uv, rectangleBlocks[i]);
        if (boxBlockD[i] <= 0.0) { color = vec3(0.0); flag = true; break; }
    }
    ...
    // Process the triangle light
    {
        ...
        for (int i = 0; i < blocks; ++i) {
            if (triLightD > boxBlockD[i]) {
                ...
                if (DoMinorArcsOverlap(lv1, lv2, v1, v2)) {
                    float dTheta = GetMinorArcsOverlapAngle(lv1, lv2, v1, v2);;
                    if (dTheta <= EPSILON) {
                        theta = 0.0;
                    }
                    else {
