...
// angle range
//
bool IsAngleInArc(float Angle, float Start, float End, float Epsilon) {
...
void main()
{
    ...
    RectangleLight rectangleLight[lights];
    BoxData        rectangleBlock[blocks];

    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            rectangleLight[i * 4 + j].Data.LeftTop     = vec2(0.2 * i, 0.2 * j) + vec2(0.15, 0.15);
            rectangleLight[i * 4 + j].Data.RightBottom = rectangleLight[i * 4 + j].Data.LeftTop + vec2(0.1, 0.1);
            rectangleLight[i * 4 + j].Color            = vec3(float(i) / 5.0, float(j) / 5.0, 0.7058823529411765);
        }
    }

    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            rectangleBlock[i * 4 + j].LeftTop     = vec2(0.2 * i, 0.2 * j) + vec2(0.3, 0.25);
            rectangleBlock[i * 4 + j].RightBottom = rectangleBlock[i * 4 + j].LeftTop + vec2(0.04, 0.04);
        }
    }
    ...
    do {
    float rectLightD[lights];
    float rectBlockD[lights];

    bool flag = false;

    for (int i = 0; i < lights; ++i) {
        rectLightD[i] = SDFBox(uv, rectangleLight[i].Data);
        if (rectLightD[i] <= 0.0) { color = rectangleLight[i].Color * 2.f; flag = true; break; }
    }
    for (int i = 0; i < blocks; ++i) {
        rectBlockD[i] = SDFBox(uv, rectangleBlock[i]);
        if (rectBlockD[i] <= 0.0) { color = vec3(0.0); flag = true; break; }
    }

    if (flag) { break; }

    vec4 lightBlockSet[lights];
    vec4 blockBlockSet[blocks];
    for (int i = 0; i < lights; ++i) {
        lightBlockSet[i] = GetFramingPointsRectangle(uv, rectangleLight[i].Data);
    }
    for (int i = 0; i < blocks; ++i) {
        blockBlockSet[i] = GetFramingPointsRectangle(uv, rectangleBlock[i]);
    }

    // Process the rectangle light
    {
        for (int i = 0; i < lights; ++i) {
            float theta = AngleBox(uv, rectangleLight[i].Data);

            for (int j = 0; j < lights; ++j) {
                if (i == j) {
                    continue;
                }
                if (rectLightD[j] < rectLightD[i]) {
                    vec4 blockSet = lightBlockSet[j];
                    vec4 lightSet = lightBlockSet[i];
            ...
            for (int j = 0; j < blocks; ++j) {
                if (rectBlockD[j] < rectLightD[i]) {
                    vec4 blockSet = blockBlockSet[j];
                    vec4 lightSet = lightBlockSet[i];
